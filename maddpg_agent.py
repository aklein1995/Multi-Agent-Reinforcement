...
class MADDPG_Agent():
    def __init__(self, state_size, action_size, num_agents, seed, \
        ...
        self.actors = [DDPG_Actor(state_size, action_size, hidden_dims=(128,128), seed = seed) for i in range(num_agents)]
        self.target_actors = [DDPG_Actor(state_size, action_size, hidden_dims=(128,128), seed = seed) for i in range(num_agents)]
    ...
    def learn(self,experiences,batch_size,idxs=0,is_weights=0):
        ...
        self._update_critic_network(experiences,batch_size,idxs,is_weights)
        if self.t_step % self.policy_update == 0:
            # *** 2. UPDATE Online Actor Networks ***
            self._update_actor_networks(experiences)
